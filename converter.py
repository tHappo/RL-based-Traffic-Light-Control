...
def cityflow2sumo_flow(args):
    ...
    output_dir = os.path.dirname(args.sumotraffic)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    sumofile = os.path.join(f_cwd, args.sumotraffic)
    cityflowfile = os.path.join(f_cwd, args.or_cityflowtraffic)

    # Verify input file exists
    if not os.path.exists(cityflowfile):
        raise FileNotFoundError(f"CityFlow traffic file not found: {cityflowfile}")
    ...
    try:
        ...
        fp.close()
        print(f"SUMO flow file generated successfully at: {sumofile}")
    except Exception as e:
        print(f"Error writing SUMO flow file: {e}")
        raise
...
def cityflow2sumo_net(args):
    ...
    sumofile = os.path.join(f_cwd, args.sumonet)
    # cityflowfile = os.path.join(f_cwd, 'data/raw_data', args.or_cityflownet)
    cityflowfile = os.path.join(f_cwd, args.or_cityflownet)
    ...
    for inter in data['intersections']:
        ...
        if not inter['virtual']:
            ...
            # Initialize yellow state
            yellow_state = ['r'] * phase_num_all
            
            # Add initial all-red phase
            phase_red = doc_node.createElement('phase')
            phase_red.setAttribute('duration', '5')
            phase_red.setAttribute('state', ''.join(['r'] * phase_num_all))
            tll.appendChild(phase_red)
            
            for idx, light in enumerate(inter['trafficLight']['lightphases']):
                state = ['r'] * phase_num_all
                if light['availableRoadLinks'] is not None:
                    # Green phase
                    if light['time'] > 5:  # Only process green phases
                        single_phase = ['G']
                        for act_roadlink in light['availableRoadLinks']:
                            state[phase_dic[act_roadlink][0]:sum(phase_dic[act_roadlink])] = single_phase*phase_dic[act_roadlink][1]
                        ...
                        yellow_state = ['r'] * phase_num_all
                        for act_roadlink in light['availableRoadLinks']:
                            yellow_state[phase_dic[act_roadlink][0]:sum(phase_dic[act_roadlink])] = ['y']*phase_dic[act_roadlink][1]
                        
                        phase_y = doc_node.createElement('phase')
                        phase_y.setAttribute('duration', '5')  # Fixed 5-second yellow phase
                        phase_y.setAttribute('state', ''.join(yellow_state))
                        tll.appendChild(phase_y)
...
def cityflow2sumo_cfg(args):
    ...
    # Ensure output directory exists
    output_dir = os.path.dirname(args.sumonet)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    # Use relative paths in the cfg file
    sumo_net = os.path.basename(args.sumonet)
    sumo_route = os.path.basename(args.sumotraffic)
    ...
    try:
        ...
        fp.close()
        print(f"SUMO cfg file generated successfully at: {sumo_cfg}")
    except Exception as e:
        print(f"Error writing SUMO cfg file: {e}")
        raise


    # Add processing element for faster simulation
    processing = doc.createElement('processing')
    threads = doc.createElement('threads')
    threads.setAttribute('value', '8')
    processing.appendChild(threads)
    root.appendChild(processing)

    # Add report element to suppress warnings
    report = doc.createElement('report')
    no_warnings = doc.createElement('no-warnings')
    no_warnings.setAttribute('value', 'true')
    report.appendChild(no_warnings)
